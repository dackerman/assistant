    ...
    switch (event.type) {
      case 'message.part.updated':
        ...
        if (part?.type === 'text') {
          ...
          return (
            <div style={{ marginLeft: '1rem' }}>
              <strong>Tool:</strong> {part.tool} | 
              <strong> Status:</strong> {part.state?.status} |
              <strong> Call ID:</strong> {part.callID}
              {part.state?.input && (
                <div style={{ marginTop: '0.5rem', fontSize: '0.9em', opacity: 0.8 }}>
                  <strong>Input:</strong> {JSON.stringify(part.state.input, null, 2)}
                </div>
              )}
              {part.state?.output && (
                <div style={{ 
                  ...
                  <pre style={{ margin: 0, whiteSpace: 'pre-wrap' }}>
                    {typeof part.state.output === 'string' 
                      ? part.state.output.substring(0, 500) + (part.state.output.length > 500 ? '...' : '')
      ...
      case 'step-start':
        return <div style={{ marginLeft: '1rem', color: '#FF9800' }}>Starting step...</div>;
