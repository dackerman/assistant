...
export function GmailToolCall({ toolCall, isExpanded, onToggle, statusIcon }: GmailToolCallProps) {
            ...
            <div>
              <div className="bg-red-50 border border-red-200 p-3 rounded">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs font-medium text-red-700">Email Details</span>
                </div>
                <div className="space-y-2 text-xs">
                  {toolCall.parameters.to && (
                    <div><span className="font-medium">To:</span> {toolCall.parameters.to}</div>
                  )}
                  {toolCall.parameters.subject && (
                    <div><span className="font-medium">Subject:</span> {toolCall.parameters.subject}</div>
                  )}
                  ...
                  {toolCall.parameters.cc && (
                    <div><span className="font-medium">CC:</span> {toolCall.parameters.cc}</div>
                  )}
                </div>
              </div>
            </div>
