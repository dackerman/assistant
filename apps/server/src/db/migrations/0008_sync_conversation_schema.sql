-- Reset conversation-related messaging tables to match application schema

-- Drop triggers and functions that depend on old columns
...
DROP FUNCTION IF EXISTS notify_message_event() CASCADE;
DROP FUNCTION IF EXISTS notify_block_created() CASCADE;
DROP FUNCTION IF EXISTS notify_block_updated() CASCADE;
DROP FUNCTION IF EXISTS notify_block_completed() CASCADE;
DROP FUNCTION IF EXISTS notify_prompt_event() CASCADE;

-- Drop dependent tables so we can recreate them with the new structure
...
-- Drop old enums that no longer match the expected values
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'tool_state') THEN
    DROP TYPE tool_state;
  END IF;
END $$;
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'prompt_status') THEN
    DROP TYPE prompt_status;
  END IF;
END $$;
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'block_type') THEN
...
-- Recreate enums using the values expected by the latest schema
...
-- Recreate tables with the expected structure
...
CREATE INDEX idx_message_queue ON messages (conversation_id, status, queue_order);
...
-- Functions and triggers for streaming notifications
