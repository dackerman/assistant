...
export class PromptService {
  ...
  private async buildConversationHistory(
    ...
    const toolCallsByMessageId = new Map<number, typeof toolCallRows>()
    for (const row of toolCallRows) {
      const existing = toolCallsByMessageId.get(row.prompt.messageId) ?? []
      existing.push(row)
      toolCallsByMessageId.set(row.prompt.messageId, existing)
    }
    ...
    for (const message of messageMap.values()) {
      ...
      if (message.role === 'assistant') {
        ...
        for (const { toolCall } of toolCallRowsForMessage) {
          contentParts.push({
            type: 'tool_use',
        ...
        for (const { toolCall } of toolCallRowsForMessage) {
          const output = sanitizeShellOutput(toolCall.output ?? '')
        ...
        const textContent = sortedBlocks
