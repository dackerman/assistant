...
import { conversations, messages, prompts, blocks, toolCalls, events, users } from "../db/schema.js";
import { eq } from "drizzle-orm";
import { setupTestDatabase, teardownTestDatabase, testDb } from "../test/setup.js";
...
describe("Tool Continuation Integration Test", () => {
  ...
  describe("Complete tool continuation flow", () => {
    it("should handle user message → AI tool call → tool execution → AI continuation", async () => {
      ...
      const { userMessageId, promptId } = await conversationService.createUserMessage(
        conversationId,
        "Find the largest file in the current directory"
      );

      // Step 3: Mock bash tool to return predictable results
      const mockToolResult = { output: "total 42\n-rw-r--r-- 1 user user 1234 test.txt", exitCode: 0 };
      ...
      const finalConversation = await conversationService.getConversation(conversationId, userId);
      
      // Should have the user message + AI response
      expect(finalConversation.messages).toHaveLength(2);
      
      const aiMessage = finalConversation.messages.find(m => m.role === "assistant");
      expect(aiMessage).toBeDefined();
      expect(aiMessage?.blocks).toBeDefined();
      
      // Should have both text blocks AND the tool call block
      const toolCallBlock = aiMessage?.blocks?.find(b => b.type === "tool_call");
      const textBlocks = aiMessage?.blocks?.filter(b => b.type === "text") || [];
...
function mockBashTool(result: { output: string; exitCode: number }) {
  ...
  vi.spyOn(ToolExecutorService.prototype, 'executeToolCall').mockImplementation(async (toolCallId: number) => {
...
async function simulateAnthropicStreamWithToolCall(
  ...
  await stateMachine.processStreamEvent({
    type: "block_delta",
    blockIndex: 0,
    delta: "I'll help you find the largest file. Let me search the current directory.",
  });
  ...
  console.log("DEBUG: Tool calls after block_end:", toolsAfterBlock.map(t => ({ id: t.id, state: t.state, toolName: t.toolName })));
  ...
  const toolsBeforeStop = await testDb
    ...
    .from(toolCalls)
    .where(eq(toolCalls.promptId, promptId));
  console.log("DEBUG: Tool calls before message_stop:", toolsBeforeStop.map(t => ({ id: t.id, state: t.state })));
  ...
  await stateMachine.processStreamEvent({
    ...
    delta: "Based on the directory listing, I can see there is a file named 'test.txt' with 1234 bytes. ",
  });
